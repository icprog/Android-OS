...
static int cdfinger_getirq_from_platform(struct cdfinger_data *cdfinger)
{
  ...
  /*  finger_int_pin = of_get_named_gpio_flags(cdfinger->spi->dev.of_node,
                      "fingerprint,touch-int-gpio", 0, NULL);
	CDFINGER_ERR("finger_int_pin = %d\n!", finger_int_pin);				  
...
static int cdfinger_parse_dts(struct cdfinger_data *cdfinger)
{
	...
	cdfinger->cdfinger_irq = pinctrl_lookup_state(cdfinger->fps_pinctrl,"eint"); // pinctrl_lookup_state函数获取dts中设置的管脚状态
...
static int cdfinger_probe(struct spi_device *spi)
{
	...
	g_cdfinger = cdfinger;
	cdfinger->spi = spi;
	if(cdfinger_parse_dts(cdfinger))  //从dts中获取数据
	{
	...
	cdfinger_power_on(cdfinger); //根据dts中的信息上电
	cdfinger_reset(100);    //感觉dts拉复位管脚
	...
	status = request_threaded_irq(cdfinger->irq, (irq_handler_t)cdfinger_interrupt_handler, NULL,
					IRQF_TRIGGER_RISING | IRQF_ONESHOT, "cdfinger-irq", NULL);
	if(status){
...
static const struct spi_device_id cdfinger_id = {SPI_DRV_NAME, 0};
...
#ifndef DTS_PROBE 
    CDFINGER_DBG("%s DTS_PROBE\n", __func__);
	err = spi_register_board_info(spi_board_cdfinger, ARRAY_SIZE(spi_board_cdfinger));
	if(err < 0){
		CDFINGER_DBG("%s spi_register_board_info err!\n", __func__);
	}else{
		CDFINGER_DBG("%s spi_register_board_info success %d!\n", __func__, err);
	}
#endif

	err = spi_register_driver(&cdfinger_driver);
	if(err < 0){
		CDFINGER_DBG("%s spi_register_driver err!\n", __func__);
	}else{
		CDFINGER_DBG("%s spi_register_driver success %d!\n", __func__, err);
	}
