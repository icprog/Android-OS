...
static struct of_device_id sof_match[] = {
        { .compatible = MA_DRV_DTS_NAME, },  
        //{ .compatible = "mediatek,hct_finger", },				//this name is used for matching the dts device for settings the gpios
};
MODULE_DEVICE_TABLE(of, sof_match);
//#endif
static struct platform_driver spdrv = {//wdong 
...
static struct of_device_id sof_dev_match[] = {
		...
		//{ .compatible = "blestech,BL229X", },						//this name is used for matching the dts device for settings the gpios
};
MODULE_DEVICE_TABLE(of, sof_dev_match);
struct spi_device_id sdev_id = {MA_DRV_NAME, 0};
struct spi_driver sdrv = {//start register spi
        .driver = {
                .name = MA_DRV_NAME,
                .bus = &spi_bus_type,
                .owner = THIS_MODULE,
...
int mas_get_interrupt_gpio(unsigned int index){
	...
	node = of_find_compatible_node(NULL, NULL, MA_DRV_EINT_DTS_NAME);
	finger_int_pin = of_get_named_gpio_flags(node, MA_INT_PIN_LABEL, 0, NULL);
	MALOGD("finger_int_pin:%d \n", finger_int_pin);
	if (!gpio_is_valid(finger_int_pin)){
		MALOGE("invalid irq gpio!");
		return -EINVAL;
