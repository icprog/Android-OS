...
static void mas_set_input(void) {
    ...
    if (!input) {
        MALOGE("input_allocate_device failed.");
        return ;
    }
    ...
    if (ret) {
        ...
        MALOGE("failed to register input device.");
        return;
    }
    smas->input  = input;
}
...
static long mas_ioctl(struct file *filp, unsigned int cmd, unsigned long arg) {
    ...
    switch(cmd){
        ...
        case ENABLE_INTERRUPT:
            enable_irq_wake(irq);//enable_irq(irq);                                                    //enable the irq,in fact, you can make irq enable always
            break;
        ...
        case WAIT_FINGERPRINTD_RESPONSE:
            ...
            mutex_unlock(&ioctl_lock);
			break;
		#if defined(FP_IOC_MAGIC) && defined(FP_IOCTL_FACTORY_TEST)
        case FP_IOCTL_FACTORY_TEST:
			{
				unsigned int chip_id = 0;
				ret = init_connect(&chip_id);//0x41
				if (1 == ret) {//1 ---> read ID succeeded 
					ret = 0; //call ioctl AND read ID succeeded !!! 
				} else {
					ret = -1; // failed 
				}
            	tmp = copy_to_user((unsigned int*)arg, &chip_id, sizeof(unsigned int));
            	MALOGD("FP_IOCTL_FACTORY_TEST:0x%x ret:%d tmp:%d chip_id:0x%x \n", FP_IOCTL_FACTORY_TEST, ret, tmp, chip_id);
			}
            break;
		#endif
...
int mas_mmap(struct file *filp, struct vm_area_struct *vma){
    ...
    MALOGF("remap_pfn_rang page:[%lu] ok.\n", page);
    return 0;
}
...
static int init_file_node(void)
{
    	...
    	if(ret < 0)
    	{
        	MALOGE("alloc_chrdev_region error!");
        	return -1;
    	}
    	sdev->chd = cdev_alloc();
   	    if (!sdev->chd)
    	{
        	MALOGE("cdev_alloc error!");
        	return -1;
    	}
...
static int init_interrupt(void)
{
    ...
    if(irq<=0){
        ret = irq;
        MALOGE("mas_get_irq irq num:%d \n", irq);
    }
    MALOGD("mas_get_irq irq num:%d \n", irq);
#ifdef DOUBLE_EDGE_IRQ
    ...
    enable_irq_wake(irq);//enable_irq(irq);
    return ret;
}
...
static int init_vars(void)
{
    ...
    if (!gWorkq) {
        MALOGE("create_single_workqueue error!");
        return -ENOMEM;
    }
    return 0;
}
 ...
 * param void
 */
int init_connect(unsigned int *chip_id){
    int i;
    MALOGD("start");
    for(i=0; i<4; i++){
    	stxb[0] = 0x8c;
		...
		*chip_id = srxb[3];
		MALOGD("guq srxb[3] = 0x%x srxb[2] = 0x%x\n", srxb[3], srxb[2]);//wdong 65(10) <==> 0x41
        if(srxb[3] == 0x41) {
			MALOGD("Compare ID OK, SPI communication has been successful!!!\n");
			return 1;
		} 
    }
    MALOGE("end");
	*chip_id = 0; //SPI communication failed, or comparison failed !!!
    return 0;   
}
#ifdef TEE_ID_COMPATIBLE

int init_connect_tee(struct spi_device *spi){
    int i;
    MALOGD("start");
    mt_spi_enable_clk();
...
int mas_plat_probe(struct platform_device *pdev) {
    MALOGD("platform driver start");//No called  ???!!!  it can also read the right ID !!!
...
int mas_probe(struct spi_device *spi) {
    ...
    MALOGD("spi driver start");
    
    //mas_plat_probe(pdev);//wdong ???
...
#ifdef REE
	#pragma message("REE is defined!!!\n") 
    ret = init_connect(&chip_id);//0;//
#elif defined TEE
	#ifdef TEE_ID_COMPATIBLE
		#pragma message("TEE_ID_COMPATIBLE is defined!!!\n") 
		ret = init_connect_tee(spi);
	#else
		ret = 1;//0;//wdong
	#endif
#endif
