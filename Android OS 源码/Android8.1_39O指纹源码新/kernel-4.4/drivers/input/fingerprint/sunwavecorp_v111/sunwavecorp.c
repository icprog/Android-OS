...
static int sunwave_factory_test(sunwave_sensor_t* sunwave_dev)
{
	//int error;
	int status=0;

	
	printk("sunwave: sunwave_factory_test_version = 0x%x\n", chip_id);
...
sunwave_dev_ioctl(struct file* filp, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd) {
        ...
        case SUNWAVE_IOC_FACTORY_TEST:
					status = sunwave_factory_test(sunwave_dev);
					printk(KERN_ERR "SUNWAVE_IOC_FACTORY_TEST, status=%d!\r\n",status);
					retval = __copy_to_user(ptr, &status, sizeof(status));
            break;
...
static int sunwave_read_id_fw(sunwave_sensor_t* sunwave_dev)
{
    ...
    do {
        ...
        while (tryTime--) {
            ...
            sw_info("*** read sunwave_fp id is 0x%4x ***\n", reg);
...
static int sunwave_read_id_8221(sunwave_sensor_t* sunwave_dev)
{
    ...
    do {
        ...
        while (tryTime--) {
            ...
            sw_info("*** read sunwave_fp id 8221 is 0x%x ***\n", reg);
...
static int sunwave_read_id_8202(sunwave_sensor_t* sunwave_dev)
{
    ...
    do {
        ...
        while (tryTime--) {
            ...
            if (sunwave_dev->finger && sunwave_dev->finger->write_then_read == 0) {
            	sw_info("*** sunwave_read_id_8202 front sunwave_dev->buffer[5]:0x%x sunwave_dev->buffer[6]:0x%x ***", sunwave_dev->buffer[5], sunwave_dev->buffer[6]);
            ...
            sw_info("*** sunwave_read_id_8202 sunwave_dev->buffer[5]:0x%x sunwave_dev->buffer[6]:0x%x ***", sunwave_dev->buffer[5], sunwave_dev->buffer[6]);

            if (reg == 0x82) {
                if (sunwave_dev->buffer[6] == 0x02) {
                    sw_info("*** read sunwave_fp id 8202 is 0x%x ***\n", reg);
                }
                else if (sunwave_dev->buffer[6] == 0x05) {
                    sw_info("*** read sunwave_fp id 8205 is 0x%x ***\n", reg);
                }
...
static int sunwave_read_id_rom(sunwave_sensor_t* sunwave_dev)
{
    ...
    do {
        ...
        romId1 = (reg << 8)  + reg1;
        romId2 = (reg1 << 8) + reg2;
        sw_info("***read sunwave_fp id rom, romId1:0x%04x, romId2:0x%04x  ***\n", romId1, romId2);
        sw_info("***read sunwave_fp id rom, buf:%02x-%02x-%02x        ***\n", reg, reg1, reg2);
        err_cnt++;
        ...
        while (tryTime--) {
            ...
            if (sunwave_dev->finger && sunwave_dev->finger->write_then_read == 0) {
                ...
                if (status <= 0) {
                   ...
                   //sunwave_dev->buffer[2]=0xa9  sunwave_dev->buffer[3]=0xba  sunwave_dev->buffer[4]=0xb7  sunwave_dev->buffer[5]=0x2b\x0a 
				   //sunwave_dev->buffer[2]=0xfb  sunwave_dev->buffer[3]=0xff  sunwave_dev->buffer[4]=0xff  sunwave_dev->buffer[5]=0x7f\x0a 
			sw_info("sunwave_dev->buffer[2]=0x%x  sunwave_dev->buffer[3]=0x%x  sunwave_dev->buffer[4]=0x%x  sunwave_dev->buffer[5]=0x%x\n",
			         sunwave_dev->buffer[2],sunwave_dev->buffer[3],sunwave_dev->buffer[4],sunwave_dev->buffer[5]);
			if (0x53 == sunwave_dev->buffer[2] && 0x75 == sunwave_dev->buffer[3] && 0x6e == sunwave_dev->buffer[4]
                && 0x57 == sunwave_dev->buffer[5]) {//sw531b
...
static int __devinit sunwave_dev_probe(struct spi_device* spi)
{
    ...
    //add by Sea 20160505
#if __SUNWAVE_DETECT_ID_EN
#pragma message("__SUNWAVE_DETECT_ID_EN is defined!!!\n") 
    status = sunwave_read_id(sunwave_dev); // sunwave read id switch default: off
