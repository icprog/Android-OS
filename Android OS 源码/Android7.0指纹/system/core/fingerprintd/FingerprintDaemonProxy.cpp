...
namespace android {
//# bird wangdong@szba-mobile.com   add start 20171019 
#if defined(BIRD_FINGERPRINT_USE_TEE_MODE)
#pragma message("BIRD_FINGERPRINT_USE_TEE_MODE is defined!!!\n") 
fjksdfjlks77777777878&&&*77777777777  //This approach is currently not validated for OK, Please debug 
...
#endif 
//# bird wangdong@szba-mobile.com   add end 20171019  
...
int64_t FingerprintDaemonProxy::openHal() {
    ...
    for(int i = 0; i < FP_VARIANT_KEYS_COUNT; i++) {
        ...
        if (0 != (err = mModule->common.methods->open(hw_module, NULL, &device))) {
            ALOGE(FPTAG"[%s,%d] Can't open fingerprint methods, error: %d",__func__, __LINE__, err);
            continue;
        }
...
#else
#pragma message("BIRD_FINGERPRINT_USE original code!!!\n") 
int64_t FingerprintDaemonProxy::openHal() {
    ALOG(LOG_VERBOSE, LOG_TAG, "nativeOpenHal()\n");
    int err;
    const hw_module_t *hw_module = NULL;
