...
#define MALOGE(x) pr_err(MALOG_TAG "[%s,%d]: error log! the function %s is failed, ret = %d\n", __func__, __LINE__, x, ret);	//error log
#define MALOGF(x) pr_err(MALOG_TAG "[%s,%d]: debug log! %s!\n", __func__, __LINE__, x);											//flag log
#define MALOGD(x) MALOGF(x)																										//debug log
#define MALOGW(x) pr_err(MALOG_TAG "[%s,%d]: warning log! the function %s's ret = %d\n", __func__, __LINE__, x, ret);			//warning log
//use for the log print
