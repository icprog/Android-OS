...
public class FingerprintService extends SystemService implements IBinder.DeathRecipient {
	...
	/* @ { bird [#14880] add by lichengfeng begin*/
	private CountDownTimer mCountdownTimer = null;
	/* @ { bird [#14880] add by lichengfeng end*/
	
	/*@ { bird [BUG #16189] changed by lichengfeng shutdown do not vibrate when touch fingerprint*/
	...
	private static boolean  mIsFingerprintInIPOState = false;
	/*@ { bird [BUG #16189] changed by lichengfeng shutdown do not vibrate when touch fingerprint*/		
	...
	/* bird @ { [BUG #16189] add by lichengfeng 20160919 begin }*/
    private final BroadcastReceiver mShutdownAndBootReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
			String action = intent.getAction();
            if (ACTION_PRE_SHUTDOWN.equals(action)) {
				android.util.Log.e("lcf_finger","FingerprintService ACTION_PRE_SHUTDOWN");
                mIsFingerprintInIPOState = true;
            } else if (NORMAL_BOOT_ACTION.equals(action)) {
				android.util.Log.e("lcf_finger","FingerprintService NORMAL_BOOT_ACTION");
				mIsFingerprintInIPOState = false;
			}
        }
    };
	/* bird @ { [BUG #16189] add by lichengfeng 20160919 end }*/
    ...
    public FingerprintService(Context context) {
		...
		/* bird @ { [BUG #16189] add by lichengfeng 20160920 begin }*/
		android.util.Log.i("lcf_finger","FingerprintService ACTION_PRE_SHUTDOWN & NORMAL_BOOT_ACTION");
		//ActivityManagerPlus.getInstance(mContext, new IPOFingerprintService());
		
		IntentFilter shutdownFilter = new IntentFilter();
		shutdownFilter.addAction(ACTION_PRE_SHUTDOWN);
		shutdownFilter.addAction(NORMAL_BOOT_ACTION);
        mContext.registerReceiver(mShutdownAndBootReceiver, shutdownFilter, null, null /* handler */);
		/* bird @ { [BUG #16189] add by lichengfeng 20160920 end }*/				
    }
	...
	/*add by lichengfeng fingerprint 20160711 begin*/
    private static final int FINGERQUICK_TYPE_DISABLE = 2;
    private static final int FINGERQUICK_TYPE_DIALOUT = 0;
    private static final int FINGERQUICK_TYPE_STARTAPP = 1;
    private static final int FINGERQUICK_TYPE_FINGERPAY = 3;	
	

    private static final String COLUMN_FINGER_ID = "fingerid";
    private static final String COLUMN_OPERATION = "operation";
    private static final String COLUMN_TARGET = "target";
    private static final String COLUMN_DATA = "data";	
	
	private void handleFingerprintOperation(Context context, int fingerId, String opPackageName) {
		android.util.Log.i("lcf_finger","FingerprintService handleFingerprintOperation opPackageName: "+opPackageName);
		if (opPackageName == null) {
            return; 
        }
		if (!opPackageName.equals(mKeyguardPackage)) {
			return; // must by Keyguard
		}
		int operation;
		String target;
		String data;
		...
		if(cursor == null) {
			operation = FINGERQUICK_TYPE_DISABLE;
			target = "";
			data = "";
		} else {
			operation = cursor.getInt(cursor.getColumnIndex(COLUMN_OPERATION));
			target = cursor.getString(cursor.getColumnIndex(COLUMN_TARGET));
			data = cursor.getString(cursor.getColumnIndex(COLUMN_DATA));
			cursor.close();
		}
		android.util.Log.i("lcf_finger","FingerprintService handleFingerprintOperation fingerId: "+fingerId);
		android.util.Log.i("lcf_finger","FingerprintService handleFingerprintOperation operation: "+operation);
		android.util.Log.i("lcf_finger","FingerprintService handleFingerprintOperation target: "+target);
		android.util.Log.i("lcf_finger","FingerprintService handleFingerprintOperation data: "+data);
		switch (operation) {
			case FINGERQUICK_TYPE_DIALOUT:
				try {
					String uriStr = "tel:"+target;
					android.util.Log.i("lcf_finger","FingerprintService handleFingerprintOperation uriStr: "+uriStr);
					Intent intent = new Intent(Intent.ACTION_CALL, Uri.parse(uriStr));
			...
			break;
			case FINGERQUICK_TYPE_STARTAPP:
				Intent intent;
				if(data == null || data.isEmpty()) {
					...
					intent.setComponent(new ComponentName(target, data));
				}
				intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_RESET_TASK_IF_NEEDED);
				context.startActivity(intent);
			break;
	...
	private final Uri FINGERQUICK_URI = Uri.parse("content://authentication.information/fingerquick");
	
	private static int getEncryptId(int origin) {
    ...
    private Cursor getFingerQuickSetting(Context context, int fingerid) {
        ...
        if(cursor == null) {
            ...
            if(cursor.getCount() > 1) {
                android.util.Log.e(TAG, "cursor count is greater than 1");
            }
            cursor.moveToFirst();
            return cursor;
        }
    }
	/*add by lichengfeng fingerprint 20160711 end*/	
    ...
    protected void handleAuthenticated(long deviceId, int fingerId, int groupId) {
        ClientMonitor client = mCurrentClient;
        if (client != null && client.onAuthenticated(fingerId, groupId) && !mIsFingerprintInIPOState /* bird @ { [bug #16189][mIsFingerprintInIPOState] add by lichengfeng 20160920 }*/) {
			// bird: changed by lichengfeng 20170417 @{
			handleFingerprintOperation(mContext, fingerId, client.getOwnerString());
			// bird: changed by lichengfeng 20170417 @}
            removeClient(client);
        }
        if (fingerId != 0) {
		...
		if (mCountdownTimer == null) {
			mCountdownTimer = new CountDownTimer(elapsedRealtimeDeadline - elapsedRealtime, 1000) {
				...
				public void onTick(long millisUntilFinished) {
					...
					if (DEBUG) android.util.Log.i("lcf_finger", "FingerprintService handleAttemptLockout secondsRemaining: "+secondsRemaining);
					notifyLockoutRemainingTimeMonitors(secondsRemaining);
				}

				@Override
				public void onFinish() {
					if (DEBUG) android.util.Log.i("lcf_finger", "FingerprintService handleAttemptLockout onFinish");
    ...
    private void scheduleLockoutReset() {
		// @ bird {add by lichengfeng 20160816 begin}
		long time = SystemClock.elapsedRealtime() + FAIL_LOCKOUT_TIMEOUT_MS;
		android.util.Log.e("lcf_finger","FingerprintService scheduleLockoutReset time: "+time);
		handleAttemptLockout(time);
		// @ bird {add by lichengfeng 20160816 end}		
	...
	/* @ { bird [#14880] add by lichengfeng begin */
    private void notifyLockoutRemainingTimeMonitors(int lockoutRemaingTime) {
        for (int i = 0; i < mLockoutMonitors.size(); i++) {
            mLockoutMonitors.get(i).sendLockoutRemainingTime(lockoutRemaingTime);
        }
    }	
	/* @ } bird [#14880] add by lichengfeng end*/	
    ...
    private void startAuthentication(IBinder token, long opId, int callingUserId, int groupId,
        ...
        AuthenticationClient client = new AuthenticationClient(getContext(), mHalDeviceId, token,
            ...
            public boolean handleFailedAttempt() {
				/* bird @ { [bug #16189] add by lichengfeng fingerprint 20160920 begin } */
				android.util.Log.v(TAG, "handleFailedAttempt mIsFingerprintInIPOState: " + mIsFingerprintInIPOState);
				if (mIsFingerprintInIPOState) {
					return false;
				}
				/* bird @ { [bug #16189] add by lichengfeng fingerprint 20160920 end } */				
                mFailedAttempts++;
    ...
    private class FingerprintServiceLockoutResetMonitor {
		...
		// @ bird { add by lichengfeng 20160816 begin }
		public void sendLockoutRemainingTime(int lockoutRemaingTime) {
            if (mCallback != null) {
                try {
                    mCallback.lockoutRemainingTime(lockoutRemaingTime);
                } catch (DeadObjectException e) {
                    Slog.w(TAG, "Death object while invoking onLockoutReset: ", e);
                } catch (RemoteException e) {
                    Slog.w(TAG, "Failed to invoke onLockoutReset: ", e);
    ...
    private final class FingerprintServiceWrapper extends IFingerprintService.Stub {
		...
		/* @ { bird [#14880] add by lichengfeng begin */
		public int getFingerprintFailedAttempts() {
			return FingerprintService.this.mFailedAttempts;
		}
		/* @ { bird [#14880] add by lichengfeng end */
		
		/* @ { bird [#16189] add by lichengfeng begin */
		public boolean getFingerprintIPOState() {
			return FingerprintService.this.mIsFingerprintInIPOState;
		}
		/* @ { bird [#16189] add by lichengfeng end */
    }
	
	/* @ { bird [#16189] add by lichengfeng begin */
	public static boolean getFingerprintIPOState() {
		return mIsFingerprintInIPOState;
	}	
	
	public static void setIsFingerprintInIPOState(boolean isFingerprintInIPOState) {
		mIsFingerprintInIPOState = isFingerprintInIPOState;
	}
	/* @ { bird [#16189] add by lichengfeng end */
