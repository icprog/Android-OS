...
/*[BIRD_PRESS_FP_SUPPORT] [fingerprint applock] lichengfeng for fps1196 @20160426 begin*/
import java.util.Arrays;
import android.app.KeyguardManager;
import android.provider.Settings;
/*[BIRD_PRESS_FP_SUPPORT] [fingerprint applock] lichengfeng for fps1196 @20160426 end*/
...
public class Activity extends ContextThemeWrapper
    ...
    protected void onResume() {
		...
		Log.i("lcf_fingerapplock", "activity---onResume");
        getApplication().dispatchActivityResumed(this);
        mActivityTransitionState.onResume(this, isTopOfTask());
        mCalled = true;
		/*[BIRD_PRESS_FP_SUPPORT] [fingerprint applock] lichengfeng for fps1196 @20160426 begin*/
            int appLockAble = Settings.System.getInt(getContentResolver(),  "com_bird_fingerprint_usedto_applock", 0);
			Log.i("lcf_fingerapplock", "activity---onResume appLockAble: " + appLockAble);
            if (appLockAble == 1) {
                String packagenameString = getPackageName();
                if (!packagenameString.equals("com.bird.fingerprintunlock") && checkIntentNull(mIntent)) {
                    String lastPackage = Settings.System.getString(getContentResolver(), "com_bird_fingerprintunlock_lastapp_package");
                    if (lastPackage != null && !lastPackage.equals(packagenameString)) {
                        ...
                        Log.i("lcf_fingerapplock", "activity---temp = " + temp);
                        if (appString != null && !temp.contains(packagenameString)) {
                            String[] appsStrings = appString.split("\\|");
                            
                            List<String> tempList = Arrays.asList(appsStrings);
                            if (tempList != null && tempList.size() > 0 && tempList.contains(packagenameString)) {
                                Log.e("lcf_fingerapplock"," need lockUI app:"+ packagenameString);
                                KeyguardManager mKeyguardManager = (KeyguardManager) getSystemService(Context.KEYGUARD_SERVICE);
                                boolean isScreenLocked = mKeyguardManager.inKeyguardRestrictedInputMode();
                                if (isScreenLocked) {
									Log.i("lcf_fingerapplock", "case 0:isScreenLocked is true, com_bird_fingerprint_usedto_screenlock is "+Settings.System.getInt(getContentResolver(), "com_bird_fingerprint_usedto_screenlock", 0));
                                    if (Settings.System.getInt(getContentResolver(), "com_bird_fingerprint_usedto_screenlock", 0) == 1) {
                                        Log.i("lcf_fingerapplock", "case 1:the lock has not unlocked, do not show the lockUI");
                                        Intent intent = new Intent("com.bird.fingerprint.update.last_package_name");
                                        intent.putExtra("packagename", packagenameString);
                                        sendBroadcast(intent);
                                    } else {
                                        Log.i("lcf_fingerapplock", "case 2:show the lockUI");
										startLockUI(packagenameString);										
                                    }
                                } else {
                                    Log.i("lcf_fingerapplock", "case 3;show the lockUI");
									startLockUI(packagenameString);
                                }
                            }
                        }
     ...
     */
	 /*[BIRD_PRESS_FP_SUPPORT] [fingerprint applock] lichengfeng for fps1196 @20160426 begin*/
    private void startLockUI(String targetPackName) {
		Intent target = new Intent();
		target.putExtra("packagename", targetPackName);
    ...
    private boolean checkIntentNull(Intent intent) {
        if(intent != null) {
            String className = intent.getComponent().getClassName();
			Log.i("lcf_fingerapplock", "checkIntentNull---className = " + className);
            if((className.equals("com.android.phone.EmergencyDialer")) 
                || (className.equals("com.android.settings.deviceinfo.WeimiUsbSettings"))
                || (className.equals("com.android.incallui.InCallActivity"))
				|| (className.equals("com.android.camera.SecureCameraActivity"))
                || (className.equals("com.android.contacts.quickcontact.QuickContactActivity"))){
                return false;
            }
        }
        return true;
        ...
        if ("1".equals(SystemProperties.get("ro.mtk_fullscreen_switch"))
            ...
            if (baseContext != null) {
                ...
                if (!isFullOn) {
                    ...
                    c.screenLayout = Configuration.reduceScreenLayout(sl, longSize, shortSize);
                    Slog.d(TAG, " getResources c.screenLayout = " + c.screenLayout, new Exception());
                }
            }
        }
        return r;
