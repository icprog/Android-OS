...
public class AliFingerSinglePreferenceFragment  extends SettingsPreferenceFragment
	...
	*/
	...
	private void initData() {
		...
		mFingerprint = (Fingerprint) intent.getExtras().getParcelable(AliFingerprintSettingsPreference.FINGERPRINT);
		...
		private RemovalCallback mRemoveCallback = new RemovalCallback() {
			...
			public void onRemovalSucceeded(Fingerprint fingerprint) {
				...
				if (mFingerprintReenrollRunningState == FINGERPRINT_REENROLL_STATE_RUNNING) {
					...
					if(getActivity()!=null){//bird:BUG #31264 add by lizhenye 20171030
					getActivity().finish();
					}
				}
			}
			...
			public void onRemovalError(Fingerprint fp, int errMsgId, CharSequence errString) {
				android.util.Log.d(TAG, "onRemovalError errMsgId: " + errMsgId + " errString: " + errString.toString());
				if (errMsgId == 5) {
					return;
				}
				showToast(errString);
            ...
            if(mOperationFromUser == AliFingerprintUtils.FINGERQUICK_TYPE_DIALOUT) {
                if(mTargetNumberFromUser == null || mTargetNumberFromUser.isEmpty()) {
                    ...
                    if(displayName != null && !displayName.isEmpty()) {
                        mFingerquickNumberPref.setSummary(displayName + "(" + mTargetNumberFromUser + ")");
                    } else {
