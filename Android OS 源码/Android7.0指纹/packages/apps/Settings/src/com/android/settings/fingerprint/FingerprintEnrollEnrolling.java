...
public class FingerprintEnrollEnrolling extends FingerprintEnrollBase
	...
	private static final int FINGERPRINT_OPEN_UNLOCK = 1;
    ...
    private void launchFinish(byte[] token) {
		...
		intent.putExtra(FINGERPRINT_REENROLL_NAME, mFingerprintReenrollName);
    ...
    public void onEnrollmentHelp(CharSequence helpString) {
		android.util.Log.i("lcf_finger","FingerprintEnrollEnrolling onEnrollmentHelp helpString=>"+helpString.toString());
		// bird @{ removed by lichengfeng 20160719 begin
		//mErrorText.setText(helpString);
		// bird @} removed by lichengfeng 20160719 end
		
		// bird @{ add by lichengfeng 20160719 begin
		showError(helpString.toString());
		// bird @} add by lichengfeng 20160719 end
    ...
    private void updateProgress(boolean animate) {
		...
		android.util.Log.i("lcf_finger", "updateProgress progress==> " + progress);
		/*add by lichengfeng custom fingerprint 20160620 begin*/
		if (progress >= 10000) {
			List<Fingerprint> enrolledlists = mFingerprintManager.getEnrolledFingerprints();
	        if(enrolledlists.size() == 1) {
				Settings.Global.putInt(getContentResolver(), "fingerprint_unlock", FINGERPRINT_OPEN_UNLOCK);
            }
			if (mFingerprintReenrollName != null) {
				Fingerprint fp = enrolledlists.get(enrolledlists.size() - 1);
				android.util.Log.i("lcf_finger", "updateProgress (mUserId != UserHandle.USER_NULL) is: " + (mUserId != UserHandle.USER_NULL));
				if (mUserId != UserHandle.USER_NULL) {
					mFingerprintManager.rename(fp.getFingerId(), mUserId, mFingerprintReenrollName);
				}
			}
		}
		/*add by lichengfeng custom fingerprint 20160620 end*/
    ...
    private final Runnable mDelayedFinishRunnable = new Runnable() {
        @Override
        public void run() {
			android.util.Log.i("lcf_finger", "FingerprintEnrollEnrolling mDelayedFinishRunnable mToken: "+mToken);
            launchFinish(mToken);
        }
    };
